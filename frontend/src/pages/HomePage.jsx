  ...
  const handleCommentSubmit = async (postId) => {
    try {
      ...
      if (response.data.success) {
        ...
        const userResponse = await get(
          `/api/v1/users/user/${newComment.userId}`,
          {
            withCredentials: true,
          }
        );
  ...
  return (
    <div className="homePageContainer">
      <div className="header">
        ...
        <p>
          This is your go-to place for amazing stories, insights, and ideas
        ...
        <div className="postsSection">
          {posts.map((post) => (
            <div key={post._id} className="postCard">
              ...
              <img
                src={`http://localhost:2000${post.image}`}
                alt={post.Title}
                className="postImage"
              />
              ...
              <div className="commentsSection">
                ...
                {post.comment.map((cmt) => (
                  <p key={cmt._id} className="comment">
                    <strong className="user">{cmt.userName}</strong>:{" "}
                    {cmt.comment}
                  </p>
                ))}
                <input
        ...
        <div className="profileSection">
          {profile ? (
            <>
              ...
              <img
                src={`http://localhost:2000${profile.profile}`}
